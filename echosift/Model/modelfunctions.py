...
from config import GEMINI_MODEL, MODEL_MAX_INPUT_TOKENS, MODEL_MAX_COMPLETION_TOKENS
...
def summarize_website_text(text: str) -> str:
    ...
    try:
        ...
        response = client.models.generate_content(
            model=GEMINI_MODEL,
            contents=[
                "You are a summarization agent. Summarize the provided content and provide analysis on extracted metadata.",
                f"Summarize this:\n{truncated}",
            ],
        )

        return response.text or "Summary could not be generated."
...
def ask_website_question(text: str, question: str) -> str:
    ...
    try:
        ...
        response = client.models.generate_content(
            model=GEMINI_MODEL,
            contents=[
                "You are an assistant that answers questions based on scraped website content.",
                f"Content:\n{truncated}\n\nQuestion: {question}",
            ],
        )
...
def summarize_pdf_text(text: str) -> str:
    ...
    try:
        ...
        response = client.models.generate_content(
            model=GEMINI_MODEL,
            contents=[
                "You are an assistant that summarizes PDF content and provides structured analysis based on extracted metadata.",
